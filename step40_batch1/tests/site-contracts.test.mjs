...
test("Build an XI is one navigation section with five focused subtabs", () => {
  ...
  assert.match(hub, /"21st Century", "century-xi"/);
  assert.match(hub, /"Present Day", "present-xi"/);
  assert.match(component, /page-continent-xi/);
  assert.match(component, /page-century-xi/);
  assert.match(component, /page-present-xi/);
});

test("era XI builders accept only their Overall Top 100 players", () => {
  const hub = read("build-xi-hub.js");
  const reader = read("reader-xi.js");
  assert.match(hub, /function top100\(era\)/);
  assert.match(hub, /overall_\$\{era\}/);
  assert.match(hub, /overall\.length < 100/);
  assert.match(hub, /Player options come only|Options come only/);
