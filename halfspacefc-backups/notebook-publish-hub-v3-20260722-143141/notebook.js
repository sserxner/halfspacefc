(() => {
  ...
  function renderEditor() {
    ...
    host.innerHTML = `<header><input data-note-title value="${esc(item.title)}" aria-label="Note title"><div><button data-note-pin>${item.pinned ? "Unpin" : "Pin"}</button><button data-note-duplicate>Duplicate</button><button data-note-archive>${item.archived ? "Restore" : "Archive"}</button><button class="danger" data-note-delete>Delete</button></div></header>
      ...
      <footer><span data-notebook-status>Saved privately</span><button data-note-save>Save version</button><button data-note-convert="transfer">Copy to Transfer</button><button data-note-convert="diary">Copy to Diary</button><button data-note-convert="editorial">Copy to Editorial</button><button data-note-convert="betting">Copy to Betting</button><button class="primary" data-note-convert="editorial" data-note-publish="true">Publish Editorial</button><button class="primary" data-note-convert="betting" data-note-publish="true">Publish Betting</button></footer>`;
  }
